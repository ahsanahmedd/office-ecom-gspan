...
//                     <div onClick={() => navigate(`/product/${item.id}`, { state: { product: item } })} className="aspect-w-1 aspect-h-1 rounded-lg overflow-hidden bg-gray-100">
//                       <img
//                         src={item.images[0]}
//                         alt={item.title}
              ...
              <div className="grid grid-cols-4 gap-3 max-w-md mx-auto">
                {product.images.map((img, index) => (
                  <button
                    key={index}
                    onClick={() => setSelectedImage(img)}
                    className={`aspect-w-1 aspect-h-1 rounded-md overflow-hidden ${selectedImage === img ? 'ring-2 ring-orange-500' : ''
                      }`}
                  >
                    <img
                      src={img}
              ...
              <nav className="flex space-x-8">
                {['Description', 'Shipping & Returns', 'Reviews'].map((tab) => (
                  <button
                    key={tab}
                    onClick={() => handleTabChange(tab)}
                    className={`pb-4 text-sm font-medium ${selectedTab === tab.toLowerCase().replace(/[^a-z]/g, '')
                        ? 'border-b-2 border-orange-500 text-orange-600'
                        : 'text-gray-500 hover:text-gray-700'
                      }`}
                  >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  {similarProducts.map((item) => (
                    <div
                      ...
                      onClick={() => navigate(`/product/${item.id}`, { state: { product: item } })} // ✅ Click event added here
                    >
